...
class ProductDetail extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.item !== prevProps.item && this.props.item !== null) { 
      this.setState({
        ...
        // Sửa lại đường dẫn lấy ID category đúng theo cấu trúc DB
  ...
  const image = this.state.imgProduct.replace(/^data:image\/[a-z]+;base64,/, ''); // remove prefix
...
apiDeleteProduct(id) {
  ...
  axios.delete('/api/admin/products/' + id, config).then((res) => {
    ...
    if (result) {
      alert('OK BABY!');
      this.apiGetProducts(); // Tải lại danh sách sau khi xóa
    } else {
      alert('SORRY BABY!');
    }
  });
...
apiGetProducts() {
  ...
  axios.get('/api/admin/products?page=' + this.props.curPage, config).then((res) => {
    ...
    if (result.products.length !== 0) {
      ...
      // Nếu trang hiện tại hết sản phẩm, tự động lùi lại 1 trang
